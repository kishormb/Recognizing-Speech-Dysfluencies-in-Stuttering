...
#     """List all audio files without conversion."""
#     files = []
...
#         st.error(f"Error loading {path}: {e} - Skipping file.")
#         return None, None
...
#         return out_path
#     except Exception as e:
#         st.error(f"Error cleaning {file_path}: {e}")
#         return None
...
#     except Exception as e:
#         st.error(f"Error in breath detection: {e}")
#         return []
...
#     except Exception as e:
#         st.error(f"Whisper error {file_path}: {e}")
#         return ""
...
#     """Generate interactive multi-class ROC curves using Plotly."""
...
#     st.write(f"Processed {processed_count} files successfully.")
...
#     st.success(f"Pipeline Complete. Processed {len(per_file_rows)} files. Check {OUTPUT_DIR} for CSVs.")
...
# Try importing tqdm, handle if missing
try:
    from tqdm import tqdm
except ImportError:
    def tqdm(iterable, **kwargs):
        return iterable  # Fallback to no progress bar
...
def load_audio_any(path: str, sr: int = TARGET_SR) -> Tuple[np.ndarray, int]:
    ...
    try:
        ...
        st.error(f"Error loading {path}: {e} - Skipping file. Ensure ffmpeg is installed for MP3 support.")
        return None, None
...
def plot_roc(y_true, y_score_dict, n_classes, class_names):
    ...
    fig.update_layout(
        ...
        hovermode='closest',
        legend=dict(x=1.02, y=0.98, xanchor='left', yanchor='top', bgcolor='rgb(15, 18, 11)', 
                    bordercolor='black', borderwidth=1, font=dict(size=12)),
        width=900,
        height=700,
        plot_bgcolor='white',
...
def main():
    ...
    st.write("### ROC Curves")
    #st.plotly_chart(roc_fig, use_container_width=True)
    st.plotly_chart(roc_fig, use_container_width=True, config={"displayModeBar": True})

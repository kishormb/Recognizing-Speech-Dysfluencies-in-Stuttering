...
# Plotting Functions
# =============================
# def plot_roc(y_true, y_score_dict, n_classes, save_path, class_names):
#     """Plot interactive multi-class ROC curves using Plotly."""
#     y_true_bin = label_binarize(y_true, classes=range(n_classes))
#     fig = go.Figure()

#     # Store traces for all curves
#     traces = []
...
#                 mode='lines',
#                 name=trace_name,
#                 line=dict(color=colors[(model_idx * n_classes + i) % len(colors)], width=2),
#                 text=[f"AUC: {roc_auc:.2f}<br>Model: {name}<br>Class: {class_names[i]}" for _ in fpr],
#                 hoverinfo='text',
#                 showlegend=True,
#                 opacity=0.6
#             )
...
#     fig.update_layout(
#         title="Interactive Multi-Class ROC Curves",
...
#             var selectedCurve = data.points[0].curveNumber;
#             var traces = plot.data;
#             for (var i = 0; i < traces.length; i++) {
#                 var update = {
#                     'line.width': (i === selectedCurve) ? 4 : 2,
#                     'opacity': (i === selectedCurve) ? 1.0 : 0.3
#                 };
...
#             full_html=True
#         ))
#     print(f"Saved interactive ROC plot to {save_path}")

def plot_roc(y_true, y_score_dict, n_classes, save_path, class_names):
    """Plot interactive multi-class ROC curves using Plotly and save AUC scores."""
    y_true_bin = label_binarize(y_true, classes=range(n_classes))
    fig = go.Figure()
    auc_scores = []  # Store AUC scores for CSV

    # Colors for traces
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2']

    # Generate ROC curves for each model and class
    for model_idx, (name, y_score) in enumerate(y_score_dict.items()):
        for i in range(n_classes):
            fpr, tpr, _ = roc_curve(y_true_bin[:, i], y_score[:, i])
            roc_auc = auc(fpr, tpr)
    ...
    # Simplified JavaScript for interactivity
    custom_js = """
    ...
    with open(save_path, 'w') as f:
        f.write(pio.to_html(
            fig,
            include_plotlyjs='https://cdn.plot.ly/plotly-2.35.0.min.js',  # Fixed version
            post_script=custom_js,
            full_html=True,
            div_id='plotly-div'  # Explicit div ID
        ))
    print(f"Saved interactive ROC plot to {save_path}")

    # Return AUC scores for CSV integration
...
def main():
    ...
    # plot_roc(y_test, y_score_dict, n_classes=len(le.classes_), save_path=roc_path, class_names=le.classes_)
    ...
    y_score_dict = probabilities
    roc_path = os.path.join(OUTPUT_DIR, "roc_curves.html")
    auc_df = plot_roc(y_test, y_score_dict, n_classes=len(le.classes_), save_path=roc_path, class_names=le.classes_)
    ...
    if "Random Forest" in models:
        ...
        # Append AUC scores to feature DataFrame
        auc_rows = []
        for _, row in auc_df.iterrows():
            auc_rows.append({
                "feature": f"AUC_{row['model']}_{row['class']}",
                "importance": row['auc']
            })
        ...
        print(f"\nTop 10 Important Features (including AUC scores):")
        print(feat_df.head(10).to_string(index=False))    
